...
export default function ProfChatbot({ context, sequenceId, teacherId }: ProfChatbotProps) {
    ...
    const cleanLatexForPreview = (latex: string) => {
        ...
        content = content
            ...
            .replace(/\\begin\{definition\}(\[.*?\])?/g, '\n<div className="border-l-4 border-blue-500 bg-blue-500/10 p-4 rounded-r-lg my-4">\n**Définition$1** : ')
            .replace(/\\end\{definition\}/g, '\n</div>\n')
            .replace(/\\begin\{propriete\}(\[.*?\])?/g, '\n<div className="border-l-4 border-green-500 bg-green-500/10 p-4 rounded-r-lg my-4">\n**Propriété$1** : ')
            .replace(/\\end\{propriete\}/g, '\n</div>\n')
            .replace(/\\begin\{exemple\}(\[.*?\])?/g, '\n<div className="border-l-4 border-orange-500 bg-orange-500/10 p-4 rounded-r-lg my-4">\n**Exemple$1** : ')
            .replace(/\\end\{exemple\}/g, '\n</div>\n')
            .replace(/\\begin\{methode\}(\[.*?\])?/g, '\n<div className="border-l-4 border-purple-500 bg-purple-500/10 p-4 rounded-r-lg my-4">\n**Méthode$1** : ')
            .replace(/\\end\{methode\}/g, '\n</div>\n')
            .replace(/\\begin\{theoreme\}(\[.*?\])?/g, '\n<div className="border-l-4 border-emerald-600 bg-emerald-600/10 p-4 rounded-r-lg my-4">\n**Théorème$1** : ')
