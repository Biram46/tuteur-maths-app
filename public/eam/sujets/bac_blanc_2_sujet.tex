% SUJET BAC BLANC – ÉPREUVE ANTICIPÉE (PREMIÈRE GT & STMG)
% Inspiré du fichier Bac blanc n°2.pdf
% Conforme au modèle modele_ndiayeAI.tex
...
\newcommand{\classe}{Première Générale et Technologique}
...
\text{Note} & 12 & 9 & 18 & x \\
\hline
\text{Coefficient} & 1 & 3 & x & 1 \\
\hline
\end{array}
\]
On note \(m\) la moyenne. Que doit valoir \(x\) pour que \(m = 14\) ?
\begin{itemize}
\item[A.] \(7\)
\item[B.] \(1\)
\item[C.] \(4\)
...
\item On note \(\mathcal{C}\) la courbe représentative de \(f\). Montrer que \(\mathcal{C}\) admet exactement deux tangentes de coefficients directeurs égaux à \(1\). \reponse
