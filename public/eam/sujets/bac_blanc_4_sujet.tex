...
\item On lance trois fois de suite un dé truqué. La probabilité d’obtenir un 6 à chaque lancer est $0,2$. La probabilité d’obtenir au moins un 6 est :
\begin{itemize}[label=$\square$]
\item[A.] $0,512$
\item[B.] $0,488$
\item[C.] $0,6$
...
\begin{center}
\begin{tikzpicture}
\begin{axis}[ybar, ymin=0, ymax=60, symbolic x coords={16,17,18,19}, xtick=data, ytick={0,10,20,30,40,50,60}, ylabel=Effectif, xlabel=Âge, grid=major, grid style={gray!30}]
...
\begin{enumerate}
    ...
    \reponse
    \item La balle touche le sol lorsque $h(t)=0$. Résoudre l’équation $-5t^2+20t+1=0$. On donnera la valeur exacte de la solution positive puis une valeur approchée au centième.
    \reponse
